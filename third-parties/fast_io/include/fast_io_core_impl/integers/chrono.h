	...
	else
	{
		...
		if constexpr (transparent)
		{
			...
			if (i < ten)
			{
				*it = char_literal_v<u8' ', char_type>;
				++it;
				*it = static_cast<char_type>(static_cast<unsigned_char_type>(
					static_cast<unsigned_char_type>(char_literal_v<u8'0', char_type>) + i));
				++it;
				return it;
			}
		}
...
inline constexpr parse_result<char_type const *> chrono_scan_year_impl(char_type const *begin, char_type const *end,
																	   I &i) noexcept
{
	I retval{};
...
inline constexpr void chrono_scan_decimal_fraction_part_rounding_impl(char_type const *begin, char_type const *end,
																	  T &retval) noexcept
{
...
chrono_scan_decimal_fraction_part_never_overflow_impl(char_type const *begin, char_type const *end, T &t) noexcept
{
