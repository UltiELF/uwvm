...
namespace fast_io::containers
{
    ...
    namespace details
    {
        ...
        template <typename T>
        inline constexpr ::std::size_t string_sso_size{
            ((string_sso_size_val<T> % 16 == 0 ? (string_sso_size_val<T> / 16) : (string_sso_size_val<T> / 16 + 1)) * 16 -
             sizeof(::fast_io::containers::details::string_internal<T>)) /
            sizeof(T)};
        ...
        inline constexpr void string_push_back_heap_grow_twice(::fast_io::containers::details::string_internal<chtype>& imp) noexcept
        {
            ...
            if(mxsz < bfsizep1) [[unlikely]] { ::fast_io::fast_terminate(); }
            ::std::size_t const bfsizep1mul2{bfsizep1 << 1u};
            chtype* ptr;
            if constexpr(typed_allocator_type::has_reallocate) { ptr = typed_allocator_type::reallocate(imp.begin_ptr, bfsizep1mul2); }
            else { ptr = typed_allocator_type::reallocate_n(imp.begin_ptr, bfsizep1, bfsizep1mul2); }
            imp = {ptr, ptr + strsize, ptr + (bfsizep1mul2 - 1u)};
        }

        template <typename allocator_type>
        inline void string_push_back_stack_to_heap_grow_twice_common(::fast_io::containers::details::string_model* imp,
                                                                     void const* first,
                                                                     ::std::size_t ssobytes,
                                                                     ::std::size_t chsz) noexcept
        {
            using untyped_allocator_type = generic_allocator_adapter<allocator_type>;
            ...
            *imp = {ptr, static_cast<::std::byte*>(ptr) + (ssobytes - chsz), static_cast<::std::byte*>(ptr) + (twobytes - chsz)};
        }

        template <typename allocator_type, ::std::integral chtype>
        inline constexpr void string_push_back_stack_to_heap_grow_twice(::fast_io::containers::details::string_internal<chtype>& imp,
                                                                        chtype const* first) noexcept
        {
            using untyped_allocator_type = generic_allocator_adapter<allocator_type>;
            using typed_allocator_type = typed_generic_allocator_adapter<untyped_allocator_type, chtype>;
            ...
            else
            {
                ...
                ::fast_io::containers::details::string_push_back_stack_to_heap_grow_twice_common<allocator_type>(
                    reinterpret_cast<::fast_io::containers::details::string_model*>(__builtin_addressof(imp)),
                    first,
                    ssobytes,
                    sizeof(chtype));
            }
        ...
        inline constexpr void string_heap_dilate_uncheck(::fast_io::containers::details::string_internal<chtype>& imp, ::std::size_t rsize) noexcept
        {
            ...
            chtype* ptr;
            if constexpr(typed_allocator_type::has_reallocate) { ptr = typed_allocator_type::reallocate(imp.begin_ptr, rsize + 1u); }
            else { ptr = typed_allocator_type::reallocate_n(imp.begin_ptr, bfsize, rsize + 1u); }
            imp = {ptr, ptr + strsize, ptr + rsize};
        }

        template <typename allocator_type, ::std::integral chtype>
        inline constexpr void
            string_stack_to_heap_dilate_uncheck(::fast_io::containers::details::string_internal<chtype>& imp, chtype const* first, ::std::size_t rsize) noexcept
        {
            using untyped_allocator_type = generic_allocator_adapter<allocator_type>;
            using typed_allocator_type = typed_generic_allocator_adapter<untyped_allocator_type, chtype>;
            constexpr ::std::size_t ssosize{::fast_io::containers::details::string_sso_size<chtype>};
    ...
    public:
        ...
        explicit constexpr basic_string(size_type n, char_type ch) noexcept
        {
            ...
            if(n < ssosize)
            {
                ...
                for(auto e{ssobuffer.buffer + n}; b != e; ++b) { *b = ch; }
                *b = 0;
            }
            else
            {
        ...
        constexpr const_pointer c_str() const noexcept { return imp.begin_ptr; }

        constexpr pointer data() noexcept { return imp.begin_ptr; }

        constexpr const_pointer data() const noexcept { return imp.begin_ptr; }

        constexpr bool is_empty() const noexcept { return imp.begin_ptr == imp.curr_ptr; }

        constexpr bool empty() const noexcept { return imp.begin_ptr == imp.curr_ptr; }

        constexpr size_type size() const noexcept { return static_cast<size_type>(imp.curr_ptr - imp.begin_ptr); }

        constexpr size_type size_bytes() const noexcept { return static_cast<size_type>(imp.curr_ptr - imp.begin_ptr) * sizeof(value_type); }

        constexpr size_type capacity() const noexcept { return static_cast<size_type>(imp.end_ptr - imp.begin_ptr); }

        constexpr size_type capacity_bytes() const noexcept { return static_cast<size_type>(imp.end_ptr - imp.begin_ptr) * sizeof(value_type); }
            ...
            back() noexcept
        {
            ...
            if(begin_ptr == curr_ptr) [[unlikely]] { ::fast_io::fast_terminate(); }
            return curr_ptr[-1];
        }
        ...
        {
            auto begin_ptr{imp.begin_ptr}, curr_ptr{imp.curr_ptr};
            if(static_cast<size_type>(curr_ptr - begin_ptr) <= pos) [[unlikely]] { ::fast_io::fast_terminate(); }
...
#endif
        [[nodiscard]] constexpr const_reference
            index_unchecked(size_type pos) const noexcept
        {
...
#endif
        [[nodiscard]] constexpr reference
            index_unchecked(size_type pos) noexcept
        {
            return imp.begin_ptr[pos];
        }
    ...
    private:
        constexpr void moveconstructorcommon(basic_string&& other) noexcept
        {
            ...
            else
            {
                this->imp = other.imp;
                other.imp.end_ptr =
                    (other.imp.curr_ptr = other.imp.begin_ptr = other.ssobuffer.buffer) + ::fast_io::containers::details::string_sso_sizem1<char_type>;
            }
        ...
        {
            size_type const size{other.size()};
            constexpr auto ssosize{::fast_io::containers::details::string_sso_size<char_type>};
            if(size < ssosize)
            {
                this->imp = {ssobuffer.buffer, ssobuffer.buffer + size, ssobuffer.buffer + ::fast_io::containers::details::string_sso_sizem1<char_type>};
            ...
            else
            {
                ...
                auto ptr{typed_allocator_type::allocate(size + 1u)};
                this->imp = {ptr, ptr + size, ptr + size};
                ::fast_io::freestanding::non_overlapped_copy_n(other.cbegin(), size, ptr);
                ptr[size] = 0;
            }
        }

    public:
        constexpr basic_string(basic_string&& other) noexcept { this->moveconstructorcommon(::std::move(other)); }
        ...
        constexpr basic_string(basic_string const& other) noexcept { this->copyconstructorcommon(other); }

        constexpr basic_string& operator= (basic_string const& other) noexcept
        {
            if(__builtin_addressof(other) == this) { return *this; }
            this->destroy();
            this->copyconstructorcommon(::std::move(other));
            return *this;
        }

        constexpr void clear() noexcept { *(this->imp.curr_ptr = this->imp.begin_ptr) = 0; }
    ...
    private:
            ...
            grow_twice() noexcept
        {
            ...
            else [[likely]] { ::fast_io::containers::details::string_push_back_heap_grow_twice<allocator_type>(this->imp); }
        }

    public:
        constexpr const_iterator cbegin() const noexcept { return this->imp.begin_ptr; }

        constexpr const_iterator cend() const noexcept { return this->imp.curr_ptr; }

        constexpr const_iterator begin() const noexcept { return this->imp.begin_ptr; }

        constexpr const_iterator end() const noexcept { return this->imp.curr_ptr; }

        constexpr iterator begin() noexcept { return this->imp.begin_ptr; }

        constexpr iterator end() noexcept { return this->imp.curr_ptr; }

        constexpr const_reverse_iterator crbegin() const noexcept { return const_reverse_iterator(this->imp.curr_ptr); }

        constexpr const_iterator crend() const noexcept { return const_reverse_iterator(this->imp.begin_ptr); }

        constexpr const_iterator rbegin() const noexcept { return const_reverse_iterator(this->imp.curr_ptr); }

        constexpr const_iterator rend() const noexcept { return const_reverse_iterator(this->imp.begin_ptr); }

        constexpr reverse_iterator rbegin() noexcept { return reverse_iterator(this->imp.curr_ptr); }

        constexpr reverse_iterator rend() noexcept { return reverse_iterator(this->imp.begin_ptr); }
...
#endif
        constexpr void
            push_back(char_type ch) noexcept
        {
            if(this->imp.curr_ptr == this->imp.end_ptr) [[unlikely]] { this->grow_twice(); }
            *this->imp.curr_ptr = ch;
            *++this->imp.curr_ptr = 0;
        }
        ...
        constexpr void
            pop_back() noexcept
        {
            if(this->imp.curr_ptr == this->imp.begin_ptr) [[unlikely]] { ::fast_io::fast_terminate(); }
            *--this->imp.curr_ptr = 0;
        }

        constexpr void pop_back_unchecked() noexcept { *--this->imp.curr_ptr = 0; }

        constexpr void reserve(size_type new_cap) noexcept
        {
            auto begin_ptr{this->imp.begin_ptr};
            if(new_cap <= static_cast<size_type>(imp.end_ptr - begin_ptr)) { return; }
            ...
            else { ::fast_io::containers::details::string_heap_dilate_uncheck<allocator_type>(this->imp, new_cap); }
        }
    ...
    public:
        constexpr ~basic_string() { this->destroy(); }
    };
    ...
    template <::std::integral chtype, typename alloctype>
    inline constexpr ::fast_io::basic_io_scatter_t<chtype> print_alias_define(io_alias_t, basic_string<chtype, alloctype> const& str) noexcept
    {
    ...
    inline constexpr auto
        strlike_construct_define(io_strlike_type_t<char_type, basic_string<char_type, allocator_type>>, char_type const* first, char_type const* last)
    {
        return basic_string<char_type, allocator_type>(first, last);
    }

    template <::std::integral char_type, typename allocator_type>
    inline constexpr auto strlike_construct_single_character_define(io_strlike_type_t<char_type, basic_string<char_type, allocator_type>>, char_type ch)
    {
    ...
    template <::std::integral chtype, typename alloctype>
    inline constexpr void
        strlike_set_curr(::fast_io::io_strlike_type_t<chtype, basic_string<chtype, alloctype>>, basic_string<chtype, alloctype>& str, chtype* p) noexcept
    {
    ...
    template <::std::integral chtype, typename alloctype>
    inline constexpr void
        strlike_reserve(::fast_io::io_strlike_type_t<chtype, basic_string<chtype, alloctype>>, basic_string<chtype, alloctype>& str, ::std::size_t n) noexcept
    {
        str.reserve(n);
    }
    ...
    template <::std::integral chtype, typename alloctype>
    inline constexpr ::fast_io::io_strlike_reference_wrapper<chtype, basic_string<chtype, alloctype>>
        io_strlike_ref(::fast_io::io_alias_t, basic_string<chtype, alloctype>& str) noexcept
    {
