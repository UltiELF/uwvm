...
inline constexpr decltype(auto) hack_scary_val(::std::basic_string<elem, traits, alloc> &str) noexcept
{
	...
	return *reinterpret_cast<scary_ptr>(reinterpret_cast<::std::byte*>(__builtin_addressof(str)) + __builtin_offsetof(model_t, _Mypair) +
											__builtin_offsetof(compress_pair_type, _Myval2));
}
