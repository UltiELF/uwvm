...
namespace fast_io
{
    ...
    namespace details
    {
        ...
        {
            if(dirp.dirp == nullptr) [[unlikely]] { return -1; }
            return dirp.fd;
        }
    ...
    public:
        ...
        constexpr native_handle_type native_handle() const noexcept { return dirp; }

        explicit constexpr operator bool () const noexcept { return dirp.dirp; }
    ...
    namespace details
    {
        ...
        inline DIR* my_dos_fdopendir(int fd) noexcept { return my_dos_opendir(::fast_io::noexcept_call(::__get_fd_name, fd)); }
    }  // namespace details
    ...
    namespace details
    {
        ...
        {
            if(dirp.dirp == nullptr) [[unlikely]] { throw_posix_error(EBADF); }
            auto const fd{dirp.fd};
            if(fd == -1) [[unlikely]] { throw_posix_error(); }
    ...
    public:
        ...
        constexpr dos_directory_file(native_hd dirp1) : dos_directory_io_observer{dirp1}
        {
        }
        ...
        dos_directory_file& operator= (dos_directory_file const& other)
        {
            ...
            if(this->dirp.dirp) [[likely]] { noexcept_call(::closedir, this->dirp.dirp); }
            this->dirp = newdir;
            return *this;
        }

        constexpr dos_directory_file(dos_directory_file&& __restrict other) noexcept : dos_directory_io_observer{other.release()} {}
    ...
    inline posix_fs_dirent drt(dos_directory_entry ndet) noexcept { return posix_fs_dirent{details::dirp_to_fd(ndet.dirp), ndet.entry->d_name}; }

    inline posix_at_entry at(dos_directory_entry ndet) noexcept { return posix_at_entry{details::dirp_to_fd(ndet.dirp)}; }
    ...
    inline constexpr ::std::uint_least64_t inode_ul64(dos_directory_entry pioe) noexcept { return 0; }
    ...
    #endif
            case DT_UNKNOWN: return file_type::unknown;
            default: return file_type::not_found;
        };
    }
    ...
    inline dos_directory_entry operator* (dos_directory_iterator pdit) noexcept { return {pdit.dirp, pdit.entry, pdit.d_namlen}; }
    ...
    inline dos_directory_iterator& operator++ (dos_directory_iterator& pdit)
    {
        ...
        */
        ...
        auto en{errno};
        if(entry == nullptr && en) { throw_posix_error(en); }
        pdit.entry = entry;
        if(entry == nullptr) { pdit.d_namlen = 0; }
        else
        {
    ...
    inline ::std::default_sentinel_t end(posix_directory_generator const&) noexcept { return {}; }

    inline constexpr bool operator== (::std::default_sentinel_t, dos_directory_iterator const& b) noexcept { return b.entry == nullptr; }

    inline constexpr bool operator== (dos_directory_iterator const& b, ::std::default_sentinel_t) noexcept { return b.entry == nullptr; }

    inline constexpr bool operator!= (::std::default_sentinel_t, dos_directory_iterator const& b) noexcept { return b.entry; }

    inline constexpr bool operator!= (dos_directory_iterator const& b, ::std::default_sentinel_t) noexcept { return b.entry; }

    inline posix_directory_generator current(posix_at_entry pate) { return {.dir_fl = dos_directory_file(posix_file(details::sys_dup(pate.fd)))}; }
    ...
    struct basic_dos_recursive_directory_iterator
    {
        ...
        explicit constexpr basic_dos_recursive_directory_iterator(dos_DIR dp) : dirp(dp) {}
    ...
    inline basic_dos_recursive_directory_iterator<StackType>& operator++ (basic_dos_recursive_directory_iterator<StackType>& prdit)
    {
        for(;;)
        {
            ...
            )
            {
                ...
                if((*name == u8'.' && name[1] == 0) || (*name == u8'.' && name[1] == u8'.' && name[2] == 0)) { continue; }
