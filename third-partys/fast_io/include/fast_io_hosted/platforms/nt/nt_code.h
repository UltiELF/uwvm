...
namespace details
{
inline constexpr ::fast_io::manipulators::scalar_flags nt_errorflags{
	.base = 16, .full = true, .floating = ::fast_io::manipulators::floating_format::fixed};
...
inline constexpr char_type *print_reserve_nt_error_impl(char_type *iter, ::std::uint_least32_t ntstatus) noexcept
{
	...
	iter = print_reserve_define(
		io_reserve_type<char_type, scalar_manip_t<::fast_io::details::nt_errorflags, ::std::uint_least32_t>>, iter,
		{ntstatus});
	*iter = char_literal_v<u8']', char_type>;
	++iter;
	return print_reserve_define_win32_error_impl<win32_family::wide_nt>(
		iter, win32::nt::rtl_nt_status_to_dos_error(ntstatus));
}
...
template <::std::integral char_type>
inline constexpr char_type *print_reserve_define(io_reserve_type_t<char_type, nt_code>, char_type *iter,
												 nt_code e) noexcept
{
